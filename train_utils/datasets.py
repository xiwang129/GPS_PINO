...
class BurgersLoader1D(object):
    def __init__(self,datapath,ntrain,sub,nx):
        dataloader =  MatReader(datapath)
        self.s = nx // sub

        self.x_train = dataloader.read_field('a')[:ntrain,::sub]
        self.y_train = dataloader.read_field('u')[:ntrain,::sub]
    
        grid = np.linspace(0, 2*np.pi, self.s).reshape(1, self.s, 1)
        grid = torch.tensor(grid, dtype=torch.float)
        self.x_train = torch.cat([self.x_train.reshape(ntrain,self.s,1), grid.repeat(ntrain,1,1)], dim=2)
